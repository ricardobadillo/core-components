    ...
    <ThemedView margin>
      <ThemedCard>
        <SectionList
          ...
          ListFooterComponent={() => (
            <ThemedText className='bg-light-background dark:bg-dark-background font-bold mb-2' type='h2'>Secciones: { houses.length }</ThemedText>
          )}
          ListHeaderComponent={() => (
            <ThemedText className='font-bold mb-2' type='h1'>Personajes</ThemedText>
          )}
          renderItem={( { item } ) => <ThemedText>{ item }</ThemedText>}
          renderSectionHeader={({ section }) => (
            <ThemedText className='bg-light-background dark:bg-dark-background p-2 rounded-xl' type='h2'>{section.title}</ThemedText>
          )}
